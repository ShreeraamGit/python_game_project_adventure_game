...
def pirate_attack_msg():
    ...
    print_pause(f"You are about to knock when the door opens and out steps a dangerous {enemy}.\n")
...
def sword_pick_msg():
    ...
    print_pause("A ranged attack that silences enemies with bolt of electricity is in your possesion.\n")

def sword_not_picked_msg():
    print_pause("Be sure you didnt pick the Sword. Anyways All The Best...\n")

def cave_repeat_msg():
    time.sleep(1)
    print_pause("Oh boy!!!!!!!. We have visited the Cave already and taken the sword of our Choice\n")
    print_pause("It is useless to come back to the cave and searching the empty place.\n")
...
def attack_with_sword():
    time.sleep(1)
    print_pause(f"As the {enemy} moves to attack, you unleash your new sword.\n")
...
def fight_run():
    ...
    while True:
        fight_run_response = input("Would you like to (1) fight or (2) run away?\n")
